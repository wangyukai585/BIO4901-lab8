...
def get_device(request="auto"):
    ...
    if d.type not in ("cuda", "mps", "cpu"):
        raise ValueError(f"Unsupported device {d}")
    try:
        x = torch.ones(2, device=d)
        _ = x.sum().item()
    except RuntimeError:
        if not automatic:
            raise
...
def memory(d):
    ...
    return {
        "gpu_allocated_mb": torch.cuda.memory_allocated(d) / 2**20
        if d.type == "cuda"
        else None,
        "gpu_peak_mb": torch.cuda.max_memory_allocated(d) / 2**20
        if d.type == "cuda"
        else None,
        "mps_allocated_mb": torch.mps.current_allocated_memory() / 2**20
        if d.type == "mps"
        else None,
...
def logger(out):
    ...
    log = logging.getLogger(str(Path(out).resolve()))
    log.setLevel(logging.INFO)
    log.propagate = False
    if not log.handlers:
        for h in (
            logging.StreamHandler(sys.stdout),
            logging.FileHandler(Path(out) / "training.log"),
        ):
...
class Monitor:
    ...
    def __init__(self, log, d, total, estimate=2.0):
        self.log, self.d, self.total = log, d, total
        self.start = time.monotonic()
        self.done = 0
        self.state = {"epoch": 0, "loss": None, "accuracy": None}
        self.stop_event = threading.Event()
        log.info(
            "device=%s estimated_seconds=%.1f (rough initial estimate; updated after first batch)",
            d,
            total * estimate,
        )
        ...
        self.log.info(
            json.dumps(
                dict(
                    ...
                    eta_s=round(elapsed / self.done * (self.total - self.done), 2)
                    if self.done
                    else None,
                    **memory(self.d),
                )
...
def parse_fasta(path):
    ...
    for rec in SeqIO.parse(path, "fasta"):
        match = re.fullmatch(
            r"(\S+)\s+(.+)-([SMU])(?:\s+(test))?", rec.description.strip()
        )
        if not match:
            raise ValueError(f"Invalid header: {rec.description}")
        acc, loc, mem, test = match.groups()
        loc = loc.replace(".", " ")
        if loc == "Cytoplasm-Nucleus":
            continue  # Ambiguous dual localization excluded from 10-way task.
        if loc == "Plastid":
            loc = "Chloroplast"  # Course label; original annotation is broader.
        if loc not in CLASSES:
            raise ValueError(f"Unknown location: {loc}")
        seq = str(rec.seq).upper()
        if not seq or re.search("[^A-Z]", seq):
            raise ValueError(f"Invalid sequence {acc}")
        rows.append(
            dict(
                id=acc,
                sequence=seq,
                label=CLASSES.index(loc),
                location=loc,
                membrane=mem,
    ...
    if df.empty or df.id.duplicated().any():
        raise ValueError("Empty FASTA or duplicate accessions")
...
def leakage_check(df):
    ...
    return {
        "train_n": len(tr),
        "test_n": len(te),
        "cross_split_identical_sequence_hashes": len(overlap),
        "cross_split_identical_sequence_pairs": int(
            sum(
                (tr.sequence_hash == h).sum() * (te.sequence_hash == h).sum()
                for h in overlap
            )
        ),
...
def cap_split(df, n, seed):
    ...
    if n < 10:
        raise ValueError("subset/eval size must be >=10 to cover 10 classes")
    # Deterministic proportional sample WITHIN the existing split; never resplit.
    order = df.assign(
        key=df.id.map(lambda x: hashlib.sha256(f"{seed}:{x}".encode()).hexdigest())
    ).sort_values("key")
...
def load_data(args):
    ...
    if (
        audit["cross_split_identical_sequence_hashes"]
        or audit["cross_split_accession_overlap"]
    ):
        raise ValueError(
            "Exact train/test overlap detected; inspect QC before training"
        )
    return (
        cap_split(df[df.split == "train"], args.subset_size, args.seed),
        cap_split(
            df[df.split == "test"],
            args.eval_size if args.eval_size is not None else args.subset_size,
            args.seed,
        ),
    )
...
def optional_int(v):
    return None if str(v).lower() in ("none", "null", "all") else int(v)
...
def parser(stage):
    ...
    p.add_argument(
        ...
        type=optional_int,
        default=None,
        help="Cap official TRAIN split; never changes split membership",
    )
    p.add_argument(
        "--eval_size",
        type=optional_int,
        default=None,
        help="Cap official TEST split; defaults to subset_size",
    )
    p.add_argument("--epochs", type=int, default=10)
    p.add_argument("--batch_size", type=int, default=4)
    p.add_argument("--device", default="auto")
    p.add_argument("--output_dir", type=Path, default=ROOT / "results" / stage)
    ...
    p.add_argument(
        ...
        type=int,
        default=512,
        help="Residues, excluding BOS/EOS; preserve both termini",
    )
...
def tokenize(tokenizer, seqs, max_length, d, perturb=False):
    ...
    if perturb:
        seqs = [
            "X" * min(10, len(s) // 4)
            + s[min(10, len(s) // 4) : len(s) - min(10, len(s) // 4)]
            + "X" * min(10, len(s) // 4)
            for s in seqs
        ]
    b = tokenizer(
        seqs, padding=True, return_tensors="pt", return_special_tokens_mask=True
    )
...
def pooled(backbone, batch):
    ...
    h = backbone(
        **{k: v for k, v in batch.items() if k != "special_tokens_mask"}
    ).last_hidden_state
...
def load_backbone(args):
    ...
    model = AutoModel.from_pretrained(
        args.model, revision=args.revision, add_pooling_layer=False
    )
...
def precision(d, requested="auto"):
    if requested == "fp32":
        return None
    if d.type == "cuda" and torch.cuda.is_bf16_supported():
...
def amp(d, dtype):
    return (
        torch.autocast(device_type=d.type, dtype=dtype)
        if dtype
...
def save_predictions(path, df, probs):
    if (
        probs.shape != (len(df), 10)
        or not np.isfinite(probs).all()
        or not np.allclose(probs.sum(1), 1, atol=1e-4)
    ):
        raise ValueError("Invalid probabilities")
    probs = probs.astype(np.float64)
    probs = probs / probs.sum(1, keepdims=True)
    out = df[["id", "label", "length", "membrane", "split"]].copy()
    out["prediction"] = probs.argmax(1)
    for k in range(10):
        out[f"p{k}"] = probs[:, k]
...
def run_metadata(args, d, **extra):
    ...
    return dict(
        config=vars(args),
        dataset_sha256=data_digest(args.data),
        device=str(d),
        platform=platform.platform(),
        torch=torch.__version__,
        transformers=transformers.__version__,
        peft=peft.__version__,
        status="SMOKE_PLACEHOLDER" if args.subset_size else "FULL_RUN",
        **memory(d),
        **extra,
    )
